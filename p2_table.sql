...
CREATE TABLE buyorder(buy_id INTEGER NOT NULL
...
CREATE TABLE sellorder(sell_id INTEGER NOT NULL
...
CREATE TABLE transactions(trans_id INTEGER NOT NULL
...
CREATE TABLE assets(ticker VARCHAR(30) NOT NULL
                   ,volume INTEGER NOT NULL
                   ,PRIMARY KEY(ticker));

--Bonds may not have a price reflected on certain dates (eg. default of bonds)
CREATE TABLE bonds(ticker VARCHAR(30) NOT NULL
...
CREATE TABLE stocks(ticker VARCHAR(30) NOT NULL
...
CREATE TABLE options(ticker VARCHAR(30) NOT NULL
                    ,strike_price DECIMAL(10,2)
                    ,pricepercontract DECIMAL(10,2)
                    ,PRIMARY KEY(ticker));
                    
CREATE TABLE companies(comp_id INTEGER NOT NULL
                      ,comp_name VARCHAR(30) NOT NULL
                      ,PRIMARY KEY(comp_id));
                      
--Maybe we can change the account name to account ID since it is no longer a weak entity?
CREATE TABLE tradingaccount(acc_id INTEGER NOT NULL
...
CREATE TABLE positions(acc_id VARCHAR(30) NOT NULL
